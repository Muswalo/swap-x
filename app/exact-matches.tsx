...
export default function ExactMatchesScreen() {
    ...
    try {
      ...
      if (profileData) {
        const matches = otherSwaps.filter(swap => {
          ...
          const locationMatch = 
            swap.current_district === profileData.current_district &&
            swap.desired_district === profileData.current_district;
          
          const ministryMatch = 
            swap.current_ministry === profileData.current_ministry;
          
          return locationMatch || ministryMatch;
        });
        setExactMatches(matches);
      } else {
        setExactMatches(otherSwaps.slice(0, 10));
      }
    } catch (error) {
      console.error('Error loading exact matches:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor: bg, paddingTop: insets.top }]}>
      <ScreenHeader
        title="Exact Matches"
        subtitle="Perfect swap opportunities for you"
